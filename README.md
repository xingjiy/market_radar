...
## 数据源与降级
- **腾讯财经（主）**：指数、ETF/个股报价（`qt.gtimg.cn`）；
- **东方财富（备 / 补充）**：涨跌家数（`ulist.np/get`）、行业板块与资金流（`clist/get`）、ETF 备源、涨停/跌停/炸板池（`push2ex`）、搜索（`searchapi`）；
- **降级链**：指数/ETF 报价 腾讯 → 东方财富 → Mock；板块/广度/涨停池 东方财富 → Mock；
- 代理层按数据域独立降级（指数失败不影响板块/ETF），`domains` 标注各域来源，`warnings[]` 上报前端；
