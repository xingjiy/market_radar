...
| Hero 市场脉冲 | 01 | 脉冲指数仪表盘（0-100）+ 核心指标网格（6 项，L2 详情） | 快照聚合 |
| A股指数 | 02 | 主要/宽基指数实时行情 + L2 详情（开高低收/成交额/振幅/成分涨跌家数） | 腾讯报价 + 东财成分涨跌 |
| 市场脉冲走势 | 03 | 今日/本周/本月时序 | 脉冲计算 |
| 市场情绪雷达 | 04 | 六维情绪因子雷达图 | 情绪计算 |
| 热点板块排行 | 05 | 板块涨幅横向条形图 + 当前聚焦 | 板块数据 |
| 板块轮动地图 | 06 | TreeMap（面积=资金规模，颜色=涨跌） | 板块数据 |
| ETF 资金雷达 | 07 | ETF 列表（价格/涨跌/成交额/资金流/信号） | ETF 数据 |
| 核心观察列表 | 08 | 自选股票/ETF 列表（搜索过滤/关注星标） | 观察池 |
| AI 市场简报 | 09 | 状态 / 摘要 / 行动建议 / 置信度 | AI 简报 |
...
> 六卡支持点击展开第二层级详情面板（`MetricGrid.vue` + `MetricDetailPanel.vue`），再次点击或点击关闭按钮收起；同一时间仅展开一个。

| 卡片 | L2 主值 | 关键分项 | 可视化 / 评级 |
| --- | --- | --- | --- |
| 上涨/下跌 | 上涨家数 | 下跌、平盘、涨跌差、上涨占比 | 涨跌平占比条 + 普涨/分化/普跌 |
| 涨停/跌停 | 涨停家数 | 跌停、炸板、封板率、涨停-跌停 | 封板/炸板占比条 + 封板强弱 |
| 两市成交额 | 两市合计 | 沪市、深市、沪市占比、较基准 | 沪深占比条 + 放量/平量/缩量 |
| 炸板率 | 炸板率 | 炸板、封板、封板率、涨停池规模 | 封板/炸板占比条 + 情绪健康/偏热/高风险 |
| 热点集中度 | 集中度分 | TOP1-3 板块、正流入板块数 | 板块资金占比排行 + 主线集中/分散 |
| 主力净流入 | 板块合计 | 净流入/流出板块数、流入/流出最大 | 净流入/流出 TOP 双榜 + 方向评级 |

**数据派生**
- 全部由 `market-snapshot` 快照字段在 `utils/metricDetails.ts`（纯函数）派生，不新增外部接口；
- 快照新增 `shTurnoverYi / szTurnoverYi`（腾讯指数报价拆分），供两市成交额分层；
- 热点集中度 = TOP3 正流入板块主力净流入 / 全部正流入板块合计（无正流入时按成交额口径，仍无数据回退 Mock 72.4）；
- 无数据/接口失败时回退 Mock，面板显示「--」或「暂无数据」。
...
- **走势分析**：MA5/10/20/60 + 多空排列 + 5/20 日动量 → 方向与 0-100 评分；附带近 60 日 K 线图（蜡烛 + 均线 + 支撑/压力标记 + 成交量）、技术指标（RSI14 / MACD / BOLL / 量能 5/20）与短/中/长期多周期评级，附走势要点与规则化摘要。
- **可选大模型诊断**：函数内置 OpenAI 兼容调用，预置两家免费档——智谱 `glm-4-flash` 与硅基流动 `Qwen/Qwen2.5-7B-Instruct`，通过 `LLM_PROVIDER`（`zhipu` / `siliconflow`，默认 `zhipu`）切换，`LLM_BASE_URL` / `LLM_MODEL` 可覆盖。配置 Netlify 环境变量 `LLM_API_KEY` 后自动启用；未配置或调用失败时回退规则分析，不影响功能。Key 仅存服务端，不暴露前端。
...
**功能概述**：展示主要与宽基指数实时行情，支持点击展开 L2 详情（与指标卡 L2 交互一致）。

**内容与交互**
- 分组：主要指数（上证指数/深证成指/创业板指/沪深300）、宽基指数（上证50/科创50/中证500）；
- L1 卡片：名称/代码、点位、涨跌额、涨跌幅（红涨绿跌）、成交额、振幅；
- L2 详情：今开/昨收/最高/最低、成交额、振幅、成分股涨跌家数占比条、趋势评级；
- 交互：segmented 分组切换、卡片点击展开/收起、键盘可访问、移动端自适应。

**数据源**
- 腾讯 `qt.gtimg.cn` 指数报价：点位/涨跌额/涨跌幅/开高低收/成交额（f31/f32/f33/f34/f37）；
- 东财 ulist（f104-106）成分涨跌家数：`push2` 主、`push2delay` 备（WAF 抗拦截）；
- 任一数据域失败独立降级，无数据显示「--」。

**验收要点**
1. 两组指数卡片数值与快照一致；
2. 点击卡片展开 L2，开高低收/成交额/成分涨跌家数正确；
3. 东财成分涨跌不可用时指数卡片不受影响；
4. 红涨绿跌、无数据兜底。
...
│   ├── IndexQuotes.vue              # A股指数（主要/宽基 + L2 详情）
